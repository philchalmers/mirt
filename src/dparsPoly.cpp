...
{		
	...
	double P[N][nzeta+2], PQfull[N][nzeta+2], Pk[N], Pk_1[N], Pk_p1[N], PQ_1[N], 
	       PQ[N], PQ_p1[N], Pk_1Pk[N], Pk_Pkp1[N], dif1[N], dif1sq[N], dif2[N], 
	       dif2sq[N], tmp1[N], tmp2[N], tmp3[N], csums[nfact], mattmp[N][nfact], 
	       d2Louter[nfact * nfact], tmp;
	int factind[nfact];
	for(j = 0; j < (nzeta + 2); j++){
		for(i = 0; i < N; i++){
			P[i][j] = prob(i,j);
	...
	for(j = 0; j < (nzeta + 1); j++){
		...
		polyOuter(d2Louter, Thetas, Pk, Pk_1, PQ_1, PQ, dif1sq, dif1);
		m = 0;
		for(k = 0; k < nfact; k++){
			for(i = 0; i < nfact; i++){
