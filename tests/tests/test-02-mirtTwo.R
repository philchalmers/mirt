...
test_that('poly', {
    ...
    expect_equal(as.numeric(resmat), c(1, -0.167, -0.144, 0.084, -0.167, 1, -0.055, -0.218, -0.144, -0.055, 1, -0.446, 0.084, -0.218, -0.446, 1))
    resmatLD <- residuals(modp3, type = 'LD', verbose = FALSE)
    expect_equal(as.numeric(resmatLD), c(NA, -20.136, -13.296, 19.57, -0.131, NA, 10.902, -21.596, -0.106, 0.096, NA, -17.522, 0.129, -0.136, -0.122, NA))
    resmatG2 <- residuals(modp3, type = 'LDG2', verbose = FALSE)
    expect_equal(as.numeric(resmatG2), c(NA, -22.905, -12.604, 23.204, -0.14, NA, 10.759, -17.256, -0.104, 0.096, NA, -18.507, 0.14, -0.121, -0.125, NA))
    cof1 <- coef(modp1)
    expect_is(cof1, 'list')
