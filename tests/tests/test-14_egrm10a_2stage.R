...
a.xi.sd.est <- sd(fscores(mod2, full.scores=T)[,2])

#============================
# Using a.si.sd.est

s.mirt.model.a <- paste("F1 = 1-10
F2 = 1-10
PRIOR = (1-10, a.xi, norm, ",a.xi.sd.est,", 1)
CONSTRAIN = (1-10, a.xi)
", sep="")
...
sd(fscores(mod3a, full.scores=T)[,2]); a.xi.sd.est <- sd(fscores(mod3a, full.scores=T)[,2])
sd(Theta[,2])
