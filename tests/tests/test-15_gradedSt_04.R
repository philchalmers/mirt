...
#   1. trying norm2p =     (tanh(2*x)+1)/2
#   2. using grmSt model for GRM data
...
# Data(R) Generation
n.subj = 1000; n.item = 20; ncat=4; # we need this for following scripts
...
#save(file="testing_grmSt5.RData", R, R2, a, d, Theta)  # subj=1000, item=20, Theta = matrix(c(rnorm(n.subj), runif(n.subj, -4, 4)), ncol=2)
#load(file="testing_grmSt5.RData")
...
mod2 <- mirt(R2, model=mirt.model(model), itemtype=c('graded', rep('grmSt2', n.item-1)), pars='values')
system.time(mod2e5 <- mirt(R2,
                         TOL=1e-5,
                         model=mirt.model(model), itemtype=c('graded', rep('grmSt2', n.item-1)), technical=list(NCYCLES=1000)))
...
par(mfcol=c(2,1))
hist(tanh(Theta[,2]/2))
hist(norm2p(fscores(mod, full.scores=T)[,2]/2), xlim=c(-1,1))
...
fs1 <- fscores(mod, full.scores=T, full.scores.SE=T)
fs2 <- fscores(mod, full.scores=T, full.scores.SE=T, rotate="none")
all.equal(fs1, fs2) # TRUE

fsML <- fscores(mod, full.scores=T, full.scores.SE=T, method="ML")
fsWLE <- fscores(mod, full.scores=T, full.scores.SE=T, method="WLE")
fsMAP <- fscores(mod, full.scores=T, full.scores.SE=T, method="MAP")
fsEAP <- fscores(mod, full.scores=T, full.scores.SE=T, method="EAP")
...
fsgrm <- fscores(modgrm, full.scores=T, full.scores.SE=T)
head(fs)
...
t1 <- Theta[,1]
t2 <- fscores(modgrm, full.scores=T)[,1]
t3 <- fscores(mod, full.scores=T)[,1]
...
pp2 <- which(fscores(mod, full.scores=T)[,2] < 1.11 & fscores(mod, full.scores=T)[,2] > 1.10)
Theta[pp2,]
