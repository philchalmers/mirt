...
# From test-16_egrmSt2.R
#   see if it works with egrmSt2Cat5, first 10 responses have no effect but to prevent mirt to drop some of the categories...

norm2p = function(x) { (tanh(2*x)+1)/2 }
...
# Data(R) Generation
n.subj = 200; n.item = 10; ncat=5; # we need this for following scripts
...
#save(file="testing_grmSt2Cat5.RData", R, R2, a, d, Theta)  # subj=500, item=20, ncat=4
#load(file="testing_grmSt2Cat5.RData")
...
R2 <- rbind(matrix(seq(1,10), nrow=10, ncol=10), R2)
R2[1:10,1]=NA
n.item=ncol(R2)
apply(R2, 2, unique)

system.time(mirt.fit <- mirt(R2, model=mirt.model(model), itemtype=c('graded', rep('grmSt2Cat5', n.item-1)),
                             technical=list(NCYCLES=2000)))

system.time(mirt.fit <- mirt(R2, model=mirt.model(model), itemtype=c('egrm10', rep('egrmSt2Cat5', n.item-1)),
                             technical=list(NCYCLES=2000)))

# ERRORS CAUSED BY R INSTEAD OF R2!!!!
...
#Called from: LoadPars(itemtype = itemtype, itemloc = itemloc, lambdas = lambdas,
#                      zetas = zetas, guess = guess, upper = upper, fulldata = fulldata,
#                      J = J, K = K, nfact = nfact + length(prodlist), parprior = parprior,
...
fs <- fscores(mirt.fit, full.scores=T, response.pattern=R2)[,"F1"]
xi <- fscores(mirt.fit, full.scores=T, response.pattern=R2)[,"F2"]
pr <- fscores(mirt.fit, full.scores=T, response.pattern=R2)[,"F3"]

cor(Theta[,1], fs[11:210])
cor(Theta[,2], xi[11:210])
cor(Theta[,3], pr[11:210])

fs.grm <- fscores(mirt(R, 1), full.scores=T, response.pattern=R)[,"F1"]

cor(Theta[,1], fs.grm)


system.time(mirt.fit <- mirt(R, model=mirt.model(model), itemtype=c('graded', rep('grmSt2Cat5', n.item-1)),
                             technical=list(NCYCLES=2000)))
...
mirt(R, model=mirt.model(model), itemtype=c('gradedCat5', rep('gradedCat5', n.item-1)),
...
R2[1:10,]=matrix(seq(10,1), nrow=10, ncol=10)
#R2[1:10,1]=1:5

system.time(mirt.fit2 <- mirt(R2, model=mirt.model(model), itemtype=c('egrm10', rep('egrmSt2Cat5', n.item-1)),
                             technical=list(NCYCLES=2000)))

save(file="testing_egrmSt2Cat5_result.RData", mirt.fit, mirt.fit2)


# comparison b2 mirt.fit and mirt.fit2...
#   they should be identical or at least similar
