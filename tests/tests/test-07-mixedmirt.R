...
test_that('mixed dich', {
    ...
    expect_equal(mod1b@Fit$df, 1001)
    ...
    expect_equal(rmod1@Fit$df, 1011)
...
test_that('polytomous', {
    ...
    expect_equal(mod@Fit$df - mod2@Fit$df, 3)
    ...
    expect_equal(as.numeric(cfs$lr.betas)[-c(1:2)], c(1.4712503, 0.2312698, 2.9707061, 0.2363475),
                 tolerance=1e-4)
    expect_equal(mod@Fit$logLik, -4360.41, tolerance = 1e-2)
    expect_equal(mod@Fit$df, 1001)
    ...
    expect_equal(mod2@Fit$logLik, -4688.784, tolerance = 1e-4)
    set.seed(1)
