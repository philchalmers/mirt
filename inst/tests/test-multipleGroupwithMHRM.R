...
test_that('three factor', {
    ...
    mod_configural <- multipleGroup(dat, models, group = group, verbose = FALSE, method = 'MHRM')
    ...
    mod_scalar1 <- multipleGroup(dat, models, group = group, verbose = FALSE,
                                 invariance=c('slopes', 'intercepts', 'free_varcov'), method = 'MHRM')    
