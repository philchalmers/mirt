...
#     expect_equal(fit$df.M2, 432, tolerance = 1e-2)
    ...
    cfs <- as.numeric(do.call(c, coef(simmod, digits=4)))
    cfs <- cfs[cfs != 0 & cfs != 1]
    expect_equal(cfs, c(1.0725, 0.2799, -1.144, 1.1505, -0.3763, 0.35, 1.2712, 0.6824, 1.1085, 0.9911, -0.3386, -2.2693, 1.0784, 0.3173, 0.3892, 0.7984, 0.9012, 0.4903, 0.6696, 0.7621, -0.5388, 0.7454, 0.9269, -0.6152, 0.9174, 0.5065, -0.5254, 0.8728, 0.7053, -0.8811, 0.8912, 0.6716, -0.4485, 1.1432, 0.2299, -0.985, 1.0039, 0.0865, -0.7764, 1.1383, -0.0919, 0.0219, 1.0785, 0.3247, 0.9125, 0.9942, 0.5972, -0.0473),
                 tolerance = 1e-2)
    fs <- fscores(simmod, scores.only=T, returnER=TRUE)
    expect_equal(as.numeric(fs), c( 0.6745527, 0.7260554, 0.1889339, 0.4748207, 0.1239444 ), tolerance = 1e-2)
})
